...
/*
    Edit of FlxTrail to stop it from updating if it's invisible
*/
...
class FlxTrail extends FlxSpriteGroup
{
	...
	/**
	 * Whether to check for visibility changes or not.
	 */
	public var visibilityEnabled:Bool = true;
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (_counter >= delay && _trailLength >= 1)
		{
			...
			if (visibilityEnabled)
				cacheValue(_recentVisible, target.visible);
			...
			for (i in 0..._recentPositions.length)
			{
				...
				if (visibilityEnabled)
					trailSprite.visible = _recentVisible[i];
	...
	public function resetTrail():Void
	{
		...
		_recentVisible.splice(0, _recentVisible.length);
