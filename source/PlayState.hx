...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var meta = Song.getMetaFile(SONG.song);
		curSongDisplayName = meta.displayName != null ? meta.displayName : SONG.song;
		...
		if (!inEditor) {
			...
			// String for when the Game Over screen appears (so you can modify it with scripts??? idk)
		...
		for (i in imagesToCheck) {
			precacheList.set(UIData.checkImageFile(i, uiSkinMap.get(i)), 'image');
		}
	...
	*/
	...
	public function setSongTime(time:Float)
	{
		...
		if (Conductor.songPosition * playbackRate <= vocals.length)
		{
			vocals.time = time * playbackRate;
		}
		vocals.play();
		if (Conductor.songPosition * playbackRate <= vocalsDad.length)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (floorSus > 0) {
					for (susNote in 0...floorSus + 1)
					{
						...
						var sustainNote:Note = new Note(ogStrum + (curStepCrochet / 2 / songSpeed), daNoteData, oldNote, true, false, keys, !isOpponent ? uiSkinMap.get('player') : uiSkinMap.get('opponent'), curStepCrochet);
		...
		if (ClientPrefs.keybindReminders && !cpuControlled) {
			...
			for (i in 0...playerKeys) {
				...
				for (j in 0...keybinds.length) {
					...
					keybindTweenGroup.set(txt, FlxTween.tween(txt, {alpha: 0}, 0.5, {startDelay: offset + 3, onComplete: function(twn) {
	...
	public function endSong():Void
	{
		...
		if (ClientPrefs.timeBarType != 'Song Name' && deathCounter < 1) songTxt.visible = false;
		canPause = false;
		endingSong = true;
		camZooming = false;
		camBop = false;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		for (i in 0...ratingsData.length) {
			if (ratingsData[i].name == daRating.name) {
				ratingNum = i;
			}
		}
		...
		rating.loadGraphic(Paths.image(UIData.checkImageFile(daRating.image, uiSkinMap.get(daRating.name))));
	...
	{
		super.stepHit();
		if (generatedMusic && (Math.abs(FlxG.sound.music.time / playbackRate - Conductor.songPosition) > 20
	...
	function resetNoteButtons() {
		...
		if (!ClientPrefs.controllerMode) {
			...
			for (i in uiSkinMap.get('player').mania) {
			...
			if (maniaData != null) {
				for (i in 0...playerKeys) {
					var button = new NoteButton(Std.int(FlxG.width / maniaData.keys) * i, 0, i, maniaData);
	...
	public function callOnScripts(event:String, args:Array<Dynamic>, ignoreStops = false, ?exclusions:Array<String>):Dynamic {
		...
		if (!inEditor) {
			...
			#if HSCRIPT_ALLOWED
			for (i in hscriptMap.keys()) {
				if(hscriptMap.get(i).closed || exclusions.contains(hscriptMap.get(i).scriptName)){
					continue;
				}
